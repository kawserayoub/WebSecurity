...
My first approach was to attempt a stacked query injection by appending a second SQL command to the URL:
Payload: http://localhost:3003/api/products/1;%20DROP%20TABLE%20products

Results: The application crashed and returned a specific error:
...
How it works: The backend likely constructs the query as DELETE FROM products WHERE id = [INPUT]. By injecting 1 OR 1=1, the logic becomes: DELETE FROM products WHERE id = 1 OR 1=1;
Since 1=1 is always true, the database ignores the specific ID and applies the deletion to every row in the table.
...
The backend constructs queries by concatenating variables directly into a string. When the input 1 OR 1=1 was provided, the sql interpreter could not distinguish between the developers intended logic and the attackers injected logic, leading to a bypass of the WHERE clause.

The application incorrectly assumed that the :id parameter would always be a simple integer. By failing to validate or sanitize the input before passing it to the database engine, the trust boundary between the untrusted user and the sensitive database was breached.

While the better-sqlite2 library blocked stacked queries, it provided to protection against union based or inferential attacks. This demonstrates that relying on a libraries default security features is not a substitute for secure coding practices.

This vulnerability is categorized under A03:2021 - Injection. Injection is ranked as the third most critical web application security risk. It occurs when untrusted data is sent to an interpreter as part of a command or query. In this lab, this resulted in a loss of confidentiality and availability when we dumped the user table and wiped the products table.
...
Lets go through each function and evaluate their vulnerabilities.

### Function 1:

![image](images/function1.png)

This function selects everything from the table. It doesn't take any input from the user. Because it doesn't combine user data and sql, there is no risk of injection. So we don't need any defense for this.

### Function 2:

![image](images/function2.png)

Here it takes the value directly from the url and places it directly to the SQL query with the help of ${}. An attacker kan change the ID in the web browser to 1 OR 1=1. The query comes WHERE id = 1 OR !=1, which returns all rows instead of just one. They can use UNION SELECT to steal data from other tables. 

A more secure version

![image](images/secure-function2.png)

The most important change is the move from a string building approach to a template driven approach. In the original code we were using javascript template literals, those backticks and the ${} syntax to construct the SQL command. The original code was was hackable in the context of a database because it merges the command and the data to a single entity before it ever reaches the database engine. This creates a situation where the database cannot tell the difference between a legitimate ID and a malicious command like 1 OR 1=!, which would trick the system into revealing every product in the table.

In this secure version we have implemented what is known as a prepared statement. Instead of baking the ID directly into the string we now use a question mark as a placeholder. Furthermore we made a subtle but an important change by switching the method from .all() to .get(). In the original version, the .all() would always return an array, even if there was only one result. However, because and ID is typically a unique identifier, we are only ever expecting a single product. When using the .get(), the library returns the product as a single object, which makes the API response much easier for the frontend to handle.

Technically this process happens in two phases. First, db.prepare(query) sends the SQL blueprint to the database engine where it is parsed and compiled. Once that structure is locked in, the second phase occurs when we call .get(params), where the engine safely plugs the data into the hole left by the question mark. This separation is the standard for database security. It ensures that the logic of our code remains unchanged and that the users input remains nothing more than passive data.

### Function 3:

![images](images/function3.png)

Here we have the same problem as the previous function but its worse because if we send 1 OR 1=1 as ID here, the database will run DELETE FROM products WHERE 1=1. We saw that this deleted the entire database in instantly.

Secure version:

![images](images/secure-function3.png)

The DELETE function is significantly more dangerous than the GET function because its misuse results in permanent loss of data rather than just a data leak. To secure the code, we replaced the the vulnerable string building (using ${}) with a prepared statement. After this fix, we are ensuring that the users input is treated only as piece of data. Which is essentially the same principle we use in the previous function.

### Help function

![images](images/helpfunction.png)

This is function is designed to accept accept only two arguments: the response object and the sql query string.

Thr main issue with this structure is that it lacks flexibility. Because the function only accepted a single query, it essentially required us to include all data, directly within the string. This design made it difficult to practice secure code. If we wanted to include a users ID or name, we were forced to use techniques like string concatenation or template literals. As we have seen, this is exactly what leads to sql injection.
...
The importance of this change lies in how it enables the use of prepared statements throughout the entire application. By adding the params array we can now use placeholder in our sql strings. When we call db.prepare(query)[type](...params), the library handles the communication with the database in a way that keeps the sql command and the user data separate.
...
In the original code, the input would have manipulated the logic to return every product in the database and delete all the products in the second image. If that were happening, we would see a larger response size containing all the product data. However the image shows a size of 0 bytes and an empty response body. For the delete request we have 0 changes which means it did not delete anything like it previously could. The database is now searching for a product that has an ID matching that exact string. With that said, we have successfully moved from a system that was not secure to a more secure one.

### Blind SQL injection

I noticed after securing this lab that the sql injections didn't return any message on the site. So I did some digging and stumbled upon a concept called "Blind SQL injection".
...
In environments where the application does not display database output or detailed errors, an attacker can still exfiltrate data by observing the servers behavior. This can be done through boolean based attacks, where the attacker checks if the page looks differently based on the true or false statement. Or time based attacks where the database is instructed to pause for a specific duration if a condition is met. Simply suppressing error messages or hiding data from the user interface doesn't truly secure the application against a determined attacker.

### Defense in depth

Beyond the code level, the database should be configured following the principle of least privilege. This principle makes sure that the application account lacks the permissions to drop tables or access system schemas like sqlite_master. Furthermore implementing strict input validation as an initial filter ensures that only expected data types, such as integers for an ID are even processed.

## 7. Risk analysis

This labs demonstrations of the vulnerabilities resulted in a total compromise of the CIA triad. The outcomes highlight that the impact of a single injection flaw extends beyond technical errors to significant risks such as data loss, legal non compliance and service disruption. This application represent a critical risk according to the OWASP Risk Rating Methodology, which evaluated risk based on the combination of the likelihood and impact.

Likelihood: High
This is high hue to the ease of discovery. The vulnerability exposed directly through the URL parameter, requiring no specialized tools or authenticated access to identify. Since the application initially returned database errors, an attacker could quickly map the database structure and execute successful injection attacks.

Impact: High
The technical impact is severe as the vulnerability allows for a complete compromise of the CIA triad. Confidentiality was lost when the UNION SELECT attack allowed unauthorized access to the users table and its sensitive credentials. Integrity was breached because of the application allowed the modification of the database logic through the URL, enabling an attacker to manipulate or bypass intended data constraints. Availability was severely impacted when the DELETE and DROP TABLE commands successfully wiped the product database, rendering the service non functional.
